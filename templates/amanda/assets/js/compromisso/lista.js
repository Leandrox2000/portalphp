...
jQuery(document).ready(function() {
    ...
    table = jQuery('#compromissos').dataTable({
        "sPaginationType": "full_numbers",
        "bSort": false,
        "fnDrawCallback": function(oSettings) {
            jQuery('#compromissos tbody input:checkbox').uniform();
        },
        "bProcessing": true,
        "bServerSide": true,
        "sAjaxSource": baseUrl() + "compromisso/pagination",
        "fnServerParams": function(aoData) {
            aoData.push({"name": "status", "value": jQuery("#status").val()});
            aoData.push({"name": "agenda", "value": jQuery("#agenda").val()});
            aoData.push({"name": "data_inicial", "value": jQuery("#dataInicial").val()});
            aoData.push({"name": "data_final", "value": jQuery("#dataFinal").val()});
        }
    });

    jQuery("#status, #agenda, #dataInicial, #dataFinal").change(function() {
        table.fnDraw();
    });
...
function alterarStatus(status){
    var sel = getCheckboxMarcados("compromissos");
...
function excluir()
{
    ...
    if (sel.length > 0) {
        jConfirm("Deseja realmente executar esta ação?", 'Confirmar Exclusão', function(response) {
            if (response) {
                jQuery.post("compromisso/delete", {sel: sel}, callBackAjax, "json");
            }
        }
        );
    } else {
