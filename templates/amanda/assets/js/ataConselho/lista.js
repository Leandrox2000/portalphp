...
jQuery(document).ready(function() {
    ...
    table = jQuery('#atas').dataTable({
        "sPaginationType": "full_numbers",
        "bSort": false,
        "fnDrawCallback": function(oSettings) {
            jQuery('#atas tbody input:checkbox').uniform();
        },
        "bProcessing": true,
        "bServerSide": true,
        "sAjaxSource": baseUrl() + "ataConselho/pagination",
...
function alterarStatus(status){
    var sel = getCheckboxMarcados("atas");
    ...
    if (sel.length > 0) {
        jConfirm(msg, 'Confirmar Alteração',
                function(response) {
                    if (response) {
                        jQuery.post("ataConselho/alterarStatus", {sel: sel, status: status}, callBackAjax, "json");
                    }
                }
        );
    } else {
...
function excluir()
{
    ...
    if (sel.length > 0) {
        jConfirm("Deseja realmente executar esta ação?", 'Confirmar Exclusão', function(response) {
            if (response) {
                jQuery.post("ataConselho/delete", {sel: sel}, callBackAjax, "json");
            }
        }
        );
    } else {
