jQuery(document).ready(function() {
    ...
    jQuery("#frm").validate({
        ...
        errorPlacement: function(error, element) {
            ...
            } else if (element.attr("name") == "pasta") {
    ...
    jQuery('#categoria').change(function() {
        jQuery.post("imagem/carregaPastasSelect/" + jQuery('#categoria').val(), function(data) {
            jQuery('#pasta').empty();
            jQuery('#pasta').html('<option selected >Selecione</option>');

            //for (row in data) {
            //    jQuery('#pasta').append('<option value=' + row + '>' + data[row] + '</option>');
            //}

            for(var i = 0; i < data.length; i++) {
                jQuery('#pasta').append('<option value=' + data[i]['id'] + '>' + data[i]['nome'] + '</option>');
            }
...
function cancelarCallback()
{
    ...
    parent.jQuery.colorbox({
        href: "imagem/imagens/checkbox/0/" + idsImgs,
        iframe: false,
        width: "75%",
        height: "90%",
        onComplete: parent.loadDataTableImagens
    });
}
