...
function adicionarStatus()
{
    if (jQuery("#novoStatus").val().trim() != "") {
        jQuery.post(baseUrl() + "edital/salvarStatus", {nome: jQuery("#novoStatus").val().trim()}, callback, "json");
    } else {
...
{
    jQuery.post(baseUrl() + "edital/getTableStatus", function(table) {
...
{
    jConfirm("Deseja realmente excluir o Status selecionado?", 'Confirmar Exclusão',
        function(response) {
            if (response) {
                jQuery.post(baseUrl() + "edital/excluiStatus", {id: id}, callback, "json");
            }
        }
    );
}
...
function editarStatus(e) {
    ...
    try {
        ...
        jQuery.post(baseUrl() + "edital/salvarStatus", params, callback, "json");
...
function salvarPosicionamento()
{
    jConfirm("Deseja realmente executar esta ação?", 'Confirmar Posição', function(response) {
        if (response) {
            var ordenacao = [];
            jQuery("#tableStatus tbody input[name='statusEditaisId']").each(function(i){
                ordenacao.push({
                    id: jQuery(this).val(),
                    ordenacao: (i + 1)
                });
            });
            jQuery.ajax('edital/ajaxAtualizarOrdenacao', {
                type: 'POST',
                dataType: 'json',
                data: {
                    ordenacao: ordenacao
