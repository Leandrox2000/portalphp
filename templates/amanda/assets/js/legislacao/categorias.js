function adicionarCategoria()
{
    if (jQuery("#novaCategoria").val().trim() != "") {
        jQuery.post(baseUrl() + "legislacao/salvarCategoria", {nome: jQuery("#novaCategoria").val().trim()}, callback, "json");
    } else {
...
{
    jQuery.post(baseUrl() + "legislacao/getTableCategorias", function(table) {
...
function excluirCategoria(id)
{
    jConfirm("Deseja realmente excluir este registro?", 'Confirmar Exclusão',
            function(response) {
                if (response) {
                    jQuery.post(baseUrl() + "legislacao/excluiCategoria", {id: id}, callback, "json");
                }
...
function editarCategoria()
{
    ...
    if (categoria != "") {
        jQuery.post(baseUrl() + "legislacao/salvarCategoria", {nome: categoria, id: id}, callback, "json");
    } else {
        jAlert("Informe a categoria.");
    }
